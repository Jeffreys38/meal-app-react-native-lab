const categoryList = [
    {
        ...
        "categoryDescription": "Beef is the culinary name for meat from cattle, particularly skeletal muscle. Humans have been eating beef since prehistoric times.[1] Beef is a source of high-quality protein and essential nutrients."
    },
    {
        "idCategory": "2",
        "categoryName": "Chicken",
        "categoryThumb": "https:\/\/www.themealdb.com\/images\/category\/chicken.png",
        "categoryDescription": "Chicken is a type of domesticated fowl, a subspecies of the red junglefowl. It is one of the most common and widespread domestic animals, with a total population of more than 19 billion as of 2011.[1] Humans commonly keep chickens as a source of food (consuming both their meat and eggs) and, more rarely, as pets."
    },
    {
        "idCategory": "3",
        "categoryName": "Dessert",
        "categoryThumb": "https:\/\/www.themealdb.com\/images\/category\/dessert.png",
        "categoryDescription": "Dessert is a course that concludes a meal. The course usually consists of sweet foods, such as confections dishes or fruit, and possibly a beverage such as dessert wine or liqueur, however in the United States it may include coffee, cheeses, nuts, or other savory items regarded as a separate course elsewhere. In some parts of the world, such as much of central and western Africa, and most parts of China, there is no tradition of a dessert course to conclude a meal.\r\n\r\nThe term dessert can apply to many confections, such as biscuits, cakes, cookies, custards, gelatins, ice creams, pastries, pies, puddings, and sweet soups, and tarts. Fruit is also commonly found in dessert courses because of its naturally occurring sweetness. Some cultures sweeten foods that are more commonly savory to create desserts."
    },
    {
        "idCategory": "4",
        "categoryName": "Lamb",
        "categoryThumb": "https:\/\/www.themealdb.com\/images\/category\/lamb.png",
        "categoryDescription": "Lamb, hogget, and mutton are the meat of domestic sheep (species Ovis aries) at different ages.\r\n\r\nA sheep in its first year is called a lamb, and its meat is also called lamb. The meat of a juvenile sheep older than one year is hogget; outside the USA this is also a term for the living animal. The meat of an adult sheep is mutton, a term only used for the meat, not the living animals. The term mutton is almost always used to refer to goat meat in the Indian subcontinent.\r\n\r\n"
    },
    {
        "idCategory": "5",
...
const mapFood = async (food: any) => {
    const { strMeal, strMealThumb, idMeal } = food;
    const description = (await fetchDetailFoodById(idMeal)).strInstructions;
...
const fetchFoodsByCategory = async (categoryName: string): Promise<any[] | null> => {
    try {
        const response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${categoryName.replaceAll(" ", '')}`);
        const foodData = await response.json();

        return await Promise.all(foodData?.meals.map(mapFood));
    } catch (error) {
        console.log(error);
        return null;
    }
...
const fetchDetailFoodById = async(foodId: string): Promise<any | null> => {
    try {
        const response = await fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${foodId}`);
        const foodData = await response.json();
        return (foodData?.meals?.length > 0) ? foodData.meals[0] : null;
    } catch (error) {
        console.log(error);
        return null;
    }
...
// (async function() {
//     let list = await fetchFoodsByCategory("Sea Food");
//     console.log(list)
// })()

export { categoryList, fetchFoodsByCategory, fetchDetailFoodById };